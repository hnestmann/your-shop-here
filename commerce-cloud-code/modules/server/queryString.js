...
var queryStringsCache = require('dw/system/CacheMgr').getCache('QueryStrings');
 ...
 * @returns {string} a stringified version of a query string oject
 **/
function toString() {
    var result = [];
    var prefKeyIdx = 1;
...
/**
 *  constructor like function returning a QueryString object from a cache and unsealing the stored object
 *  @param {string} raw the query portion of the request in unprocessed string form
 *  @returns {Object} object with convenience methods and properties for query string usage
 */
function cachedQueryString(raw) {
    var key = raw || 'empty';
    var cacheResult = queryStringsCache.get(key, function () {
        return (new QueryString(raw));
    });

    // cache results are sealed, we want to wite on them
    var result = {};
    Object.keys(cacheResult).forEach(function (name) {
        result[name] = cacheResult[name];
    });

    // for manual created query strings
...
module.exports = cachedQueryString;
