...
function parseForm(form) {
    ...
    Object.keys(form).forEach(function (key) {
        // Push keys into property to make it iterable
        // Don't set `enumerable: true`, this will result in multiple errors across the shop
        // eslint-disable-next-line no-underscore-dangle
        result._keys.push(key);

        Object.defineProperty(result, key, {
            configurable: true,
            get: function () {
                if (Object.prototype.hasOwnProperty.call(result._elements, key)) { // eslint-disable-line no-underscore-dangle
                    return result._elements[key]; // eslint-disable-line no-underscore-dangle
                }
                ...
                if (form[key] instanceof formField) {
                    ...
                    value = action(form[key]);
                } else if (form[key] instanceof formGroup) {
                    value = parseForm(form[key]);
                }

                if (value) {
                    result._elements[key] = value; // eslint-disable-line no-underscore-dangle
                }
