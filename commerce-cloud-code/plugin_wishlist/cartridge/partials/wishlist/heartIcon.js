...
const heartIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">
...
exports.template = (pid) => `
    <div class="wishlist-icon wishlist-pid-${pid}" 
        hx-get="${URLUtils.url('Wishlist-Event', 'pid', pid, 'event', 'add')}" 
        hx-swap="none"
        onclick="wishlistIconChangeState(this)"
        data-event-url-add="${URLUtils.url('Wishlist-Event', 'pid', pid, 'event', 'add')}"
        data-event-url-remove="${URLUtils.url('Wishlist-Event', 'pid', pid, 'event', 'remove')}">
        ${heartIconSvg}
    </div>
    `;
